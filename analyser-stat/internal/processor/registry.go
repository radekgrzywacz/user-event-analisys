...
type Handler interface {
	Domain() string
	Handle(envelope contracts.Envelope, rdb *redis.Client, producer *Producer) error
}
...
func (r *Registry) Handle(envelope contracts.Envelope, rdb *redis.Client, producer *Producer) error {
	handler, ok := r.handlers[envelope.Domain]
	if !ok {
		return fmt.Errorf("%w: %s", ErrUnknownDomain, envelope.Domain)
	}
	return handler.Handle(envelope, rdb, producer)
}
